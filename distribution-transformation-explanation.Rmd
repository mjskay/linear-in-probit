...
plot_lpr_grid_probit = lpr_grid |>
  ...
  # use segment for convenience in post hoc edit
  geom_segment(x = -2.59, xend = 2.59, y = -2.59, yend = 2.59, alpha = .25, linetype = "22", color = 'black') +
  scale_x_continuous() + 
  scale_y_continuous() + 
  scale_color_gradient(low = Color_Dict$dark_gradient, high = Color_Dict$text) + 
...
```{r}
mu = .475 # two reasons: the quantile df will contain this mode/mean, and the difference will be bigger
...
lpr_plot = function(X_perceived, X_displayed) {
  ...
  lpr_df |>
    ...
    geom_line(size = 1.25, color = Color_Dict$text) +
    geom_segment_(
